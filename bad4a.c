...
MinHeap sleepHeap;
// create a list of mboxes for each terminal
int readMbox[4];
...
void termReadSyscall(USLOSS_Sysargs *args) {
    ...
    // successful input, create mailbox with 10 slots, each slot with a size of MAXLINE
    // Allows storage of 10 messages that are MAXLINE bytes long
    int mbox = MboxCreate(10, MAXLINE);
    // create line we are going to read
    char line[MAXLINE];
    // store message in mailbox
    int retval = MboxRecv(mbox, &line, MAXLINE);
    if (retval > len) {
        retval = len;
    }
    args->arg2 = (int)(long) retval;
    args->arg4 = (void *) 0;
    unlock();
}
...
void handle_one_terminal_interrupt(int unit, int status) {
    ...
    if (USLOSS_TERM_STAT_RECV(status) == USLOSS_DEV_READY) {
        ...
        if (c == '\n' || next == MAXLINE) {
            ...
            MboxSend(readMbox[unit], line, next);
            USLOSS_Console("Sent\n");
        }
    }
    // if xmit is ready
...
void phase4_init() {
    ...
    for (int i = 0; i < 4; i++) {
        readMbox[i] = MboxCreate(10, MAXLINE);
    }
