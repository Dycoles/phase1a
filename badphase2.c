    ...
    if (thisBox->blockedConsumers == NULL && outOfSlots(thisBox->slots, thisBox->numSlots)) {
        ...
        if (condition == 1) {   // conditional send
            ...
            if (notInProcessQueue(thisBox->blockedProducers, &process_table[getpid() % MAXPROC])) {
                USLOSS_Console("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW\n");
    ...
    if (thisBox->blockedConsumers != NULL) {
        ...
        for (int i = 0; i < MAXSLOTS; i++) {
            if (mail_slot[i].status == 0) {
                ...
                newSlot->orderReceived = nextOrderReceived++;
                break;
            }
        }
...
static int receive(int mailboxID, void *message, int maxMessageSize, int condition) {
    ...
    // Receive message:
    if (thisBox->slots == NULL && thisBox->blockedProducers == NULL) {
    ...
    int messageFoundLen;
    int pidOfRecvd;
    thisBox->blockedProducers->slotPointer->orderReceived < thisBox->slots->orderReceived;
    USLOSS_Console("aefa\n");
    if (thisBox->slots == NULL && (thisBox->blockedProducers == NULL || thisBox->blockedProducers->slotPointer == NULL)) {

    } else if (thisBox->blockedProducers == NULL || thisBox->blockedProducers->slotPointer == NULL) {
        USLOSS_Console("aefa1\n");
        struct mailSlot *slot = dequeueSlot(thisBox->slots);
        if (slot == thisBox->slots) {
            thisBox->slots = NULL;
        }
        messageFound = slot->message;
        messageFoundLen = slot->messageSize;
        USLOSS_Console("%d\n",slot->pidProducedBy);
        pidOfRecvd = slot->pidProducedBy;
        USLOSS_Console("aefa1.1\n");
        pidOfRecvd = slot->pidProducedBy;
    } else if (thisBox->slots == NULL || thisBox->blockedProducers->slotPointer->orderReceived < thisBox->slots->orderReceived) {
        USLOSS_Console("aefa2\n");
        struct shadowTable *producer = dequeueProcess(thisBox->blockedProducers);
        if (producer == thisBox->blockedProducers) {
            thisBox->blockedProducers = NULL;
        }
        messageFound = producer->slotPointer->message;
        messageFoundLen = producer->slotPointer->messageSize;
        pidOfRecvd = producer->pid;
    } else {
        USLOSS_Console("aefa3\n");
        struct mailSlot *slot = dequeueSlot(thisBox->slots);
        if (slot == thisBox->slots) {
            thisBox->slots = NULL;
        }
    ...
    USLOSS_Console("outside\n");
    // A message is now available, so receive it:
    if (messageFound == NULL) {
        USLOSS_Console("eafe\n");
        memcpy(message, NULL, 0);
        USLOSS_Console("UnblockingA %d\n", pidOfRecvd);
        dumpProcesses();
        unblockProc(pidOfRecvd);
        dumpProcesses();
        restoreInterrupts(oldpsr);
        return 0;
    } else {
        // check for insufficient buffer size
        if (messageFoundLen > maxMessageSize) {
            //char oldPartition = messageFound[maxMessageSize-1];    // TODO -2?
            //messageFound[maxMessageSize-1] = '\0';
            USLOSS_Console("deeath\n");
            if (message != NULL) memcpy(message, messageFound, maxMessageSize-1);
            //((char*)message)[maxMessageSize-1] = '\0';
            //messageFound[maxMessageSize-1] = oldPartition;
            USLOSS_Console("UnblockingB %d\n", pidOfRecvd);
            dumpProcesses();
            unblockProc(pidOfRecvd);
            dumpProcesses();
            restoreInterrupts(oldpsr);
            ...
            USLOSS_Console("awfawf\n");
            if (message != NULL) memcpy(message, messageFound, messageFoundLen);
            USLOSS_Console("UnblockingC %d %s\n", pidOfRecvd, messageFound);
            dumpProcesses();
            unblockProc(pidOfRecvd);
            dumpProcesses();
            restoreInterrupts(oldpsr);
